...
<!-- Lucene search -->
		  ...
		  <input type="submit" value="Search" name="Search" i18n:attr="value"/>
                  ...
                  <xsl:when test="$config/search/@provider = 'search-hadoop'">
<!-- Search-Hadoop search -->
                    <xsl:variable name="search-hadoop-prompt">
                      <i18n:text>Search with Apache Solr</i18n:text>
                    </xsl:variable>
                    <form method="get" action="http://search-hadoop.com/{$config/search/@name}">
                      <input type="text" id="query" name="q" size="25" onFocus="getBlank (this, '{$search-hadoop-prompt}');">
                        <xsl:attribute name="value">
                          <xsl:value-of select="$search-hadoop-prompt"/>
                        </xsl:attribute>
                      </input>&#160;
...
<![CDATA[var gaJsHost = (("https:" == document.location.protocol) ? "https://ssl." : "http://www.");
