 ...
 * <p>
 * Event queue handles a collection of event handlers and routes the incomming
...
public class EventQueue implements EventPublisher, AutoCloseable {
  ...
  private Map<Event, Map<EventExecutor, List<EventHandler>>> executors =
      new HashMap<>();
  ...
  public <PAYLOAD, EVENT_TYPE extends Event<PAYLOAD>> void addHandler(
    ...
    this.addHandler(event, new SingleThreadExecutor<PAYLOAD>(
  ...
  /**
   * Send event to the event to the right listener.
   *
   * @param event   The event identifier
   * @param payload The payload of the event.
   * @throws IllegalArgumentException If no there is no EventHandler for
   *                                  the specific event.
   */
  public <PAYLOAD, EVENT_TYPE extends Event<PAYLOAD>> void fireEvent(
    ...
    if (eventExecutorListMap != null) {
      for (EventExecutor executor : eventExecutorListMap.keySet()) {
        for (EventHandler handler : eventExecutorListMap.get(executor)) {
          queuedCount.incrementAndGet();
          executor.onMessage(handler, payload, this);
        }
      }
  ...
  public void close() {
    ...
    Set<EventExecutor> allExecutors = this.executors.values().stream()
        .flatMap(handlerMap -> handlerMap.keySet().stream()).collect
            (Collectors.toSet());
